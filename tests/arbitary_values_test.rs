...
fn test_handles_simple_conflicts_with_arbitrary_values_correctly() {
    assert_eq!(tw_merge(String::from("m-[2px] m-[10px]")), "m-[10px]");
    assert_eq!(
        tw_merge(String::from("m-[2px] m-[11svmin] m-[12in] m-[13lvi] m-[14vb] m-[15vmax] m-[16mm] m-[17%] m-[18em] m-[19px] m-[10dvh]")),
        "m-[10dvh]"
    );
    assert_eq!(
    ...
    assert_eq!(tw_merge(String::from("z-20 z-[99]")), "z-[99]");
    assert_eq!(tw_merge(String::from("my-[2px] m-[10rem]")), "m-[10rem]");
    assert_eq!(
        tw_merge(String::from("cursor-pointer cursor-[grab]")),
        "cursor-[grab]"
    );
    assert_eq!(
        tw_merge(String::from("m-[2px] m-[calc(100%-var(--arbitrary))]")),
        "m-[calc(100%-var(--arbitrary))]"
    );
    assert_eq!(
        tw_merge(String::from("m-[2px] m-[length:var(--mystery-var)]")),
        "m-[length:var(--mystery-var)]"
    );
    assert_eq!(
        tw_merge(String::from("opacity-10 opacity-[0.025]")),
        "opacity-[0.025]"
    );
    assert_eq!(
        tw_merge(String::from("scale-75 scale-[1.7]")),
        "scale-[1.7]"
    );
    assert_eq!(
        tw_merge(String::from("brightness-90 brightness-[1.75]")),
        "brightness-[1.75]"
    );

    // Handling of value `0`
    assert_eq!(
        tw_merge(String::from("min-h-[0.5px] min-h-[0]")),
        "min-h-[0]"
    );
    assert_eq!(
        tw_merge(String::from("text-[0.5px] text-[color:0]")),
        "text-[0.5px] text-[color:0]"
    );
    assert_eq!(
        tw_merge(String::from("text-[0.5px] text-[--my-0]")),
...
fn test_handles_arbitrary_length_conflicts_with_labels_and_modifiers_correctly() {
    assert_eq!(
        tw_merge(String::from("hover:m-[2px] hover:m-[length:var(--c)]")),
...
fn test_handles_complex_arbitrary_value_conflicts_correctly() {
    assert_eq!(
        tw_merge(String::from("grid-rows-[1fr,auto] grid-rows-2")),
        "grid-rows-2"
    );
    assert_eq!(
        tw_merge(String::from(
            "grid-rows-[repeat(20,minmax(0,1fr))] grid-rows-3"
        )),
        "grid-rows-3"
    );
}
...
fn test_handles_ambiguous_arbitrary_values_correctly() {
    assert_eq!(
        tw_merge(String::from("mt-2 mt-[calc(theme(fontSize.4xl)/1.125)]")),
        "mt-[calc(theme(fontSize.4xl)/1.125)]"
    );
    assert_eq!(
        tw_merge(String::from("p-2 p-[calc(theme(fontSize.4xl)/1.125)_10px]")),
        "p-[calc(theme(fontSize.4xl)/1.125)_10px]"
    );
    assert_eq!(
        tw_merge(String::from("mt-2 mt-[length:theme(someScale.someValue)]")),
        "mt-[length:theme(someScale.someValue)]"
    );
    assert_eq!(
        tw_merge(String::from("mt-2 mt-[theme(someScale.someValue)]")),
